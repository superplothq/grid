...
```ts
import { createChartRenderer } from "@superplot/grid/renderer";
...
import type { CellProps } from "@superplot/react";
...
## Conditional renderers via matching rules
...
`grid.matchAll`. It matches facet cells by dimension name and value; chain `.prop` to
...
1. **Matching rule override** - a matching `matchAll` rule that supplies a
