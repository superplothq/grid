...
The group banner doubles as a selection target. `grid.matchAll` matches the
**Extra expense** group, `matchAllCell` narrows the selection to the data cells
...
```ts
grid
  .matchAll((_dim, value) => value === "Extra expense")            // the group's header
...
predicate** (`matchAllCell`); a facet-only `matchAll(...).style(...)` styles the
group header instead. Re-flagging on a new threshold is `active.undo()` then a
fresh matching, so the old red never lingers.
