...
`DuckDBWasmDataSource` is the recommended [SqlDataSource](/docs/datasource/sql-datasource) implementation for browser applications, to load full CSV / JSON data in memory. It runs [DuckDB](https://duckdb.org) compiled to WebAssembly directly in the browser - no server required.
...
Once created, use the inherited `loadData()` and `loadDataFromURL()` methods from [SqlDataSource](/docs/datasource/sql-datasource):
...
Extends [SqlDataSource](/docs/datasource/sql-datasource#api-reference).
