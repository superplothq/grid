...
`FlattenedDataViewModel` extends [GridDataViewModel](/docs/viewmodel). It adds a single-level row facet for group labels and packed per-row metadata for hierarchical grouping with expand/collapse. Constructed from [`FlattenedDataViewModelParams`](/docs/api-references/type-references#flatteneddataviewmodelparams).
...
Unlike [PivotDataViewModel](/docs/viewmodel/pivot-data-viewmodel) which has multi-level row facets (one level per dimension), `FlattenedDataViewModel` has a single-level row facet. All grouping depth information is encoded in the row metadata instead.
...
`getSlice` unpacks these bytes into [`FlatRowMeta`](/docs/api-references/type-references#flatrowmeta) objects for the visible slice only - the slice is small (only the visible rows), so unpacking is cheap.
...
4. DataModel returns new [`FlattenedDataViewModelParams`](/docs/api-references/type-references#flatteneddataviewmodelparams) with EU's children included
...
`getSlice` returns a [`FlatSliceResult`](/docs/api-references/type-references#flatsliceresult) which extends [`BaseSliceResult`](/docs/api-references/type-references#basesliceresult) with:
...
Same pattern as [PivotDataViewModel](/docs/viewmodel/pivot-data-viewmodel#updatedata) - swaps internal data arrays and pagination state without constructing a new instance. `metaState` is preserved.
...
See [StandardTableDataModel](/docs/datamodel/standard-table-datamodel) for how pages are managed and fetched.

## API Reference.

Extends [GridDataViewModel](/docs/viewmodel#api-reference).
