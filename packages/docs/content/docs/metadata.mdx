---
title: Metadata
description: How to attach and consume metadata - data about your data - alongside the grid's main data pipeline. Metadata flows through a resolver/reshaper pattern in the DataModel, gets stored on the ViewModel, and arrives at cell renderers for custom visualization.
---
...
```
DataModel.getViewModelData(config)
  │
  ├─ resolver.resolve(input)       ← you define: what extra data to fetch
  │    └─ e.g. SQL expressions appended to the query
  │
  ├─ getData(ir, resolver)         ← DataModel fetches data + metadata together
  │
  ├─ reshaper.reshape(input, metadata)  ← you define: map raw results → ViewModel coords
  │
  └─ returns params with metadata
        │
        ▼
  ViewModel.updateData(params)     ← metadata stored, Map indexes built
        │
        ▼
  Renderer calls cellRenderer(data, dataCtx, ctx)
        │
        └─ dataCtx.viewModel.metadata?.getValueCellMeta(col, row)
             └─ your renderer reads metadata and styles the cell
```
...
The plumber closes over whatever setup it needs and reads `config.metadata` to decide what to produce. `PivotConfig.metadata` is `unknown` - core passes it through without interpretation.

### Resolver

The resolver tells the DataModel what extra data to fetch alongside the main query. For the SQL pipeline, it returns `SqlSelectExpression[]` - SQL fragments appended to the SELECT clause:
...
`StandardDataFetchAndTransformIR` gains a `metadata?: unknown` field - same pattern as `PivotConfig.metadata`. The plumber reads `ir.metadata` to decide what to produce.
...
When the user scrolls to new rows, `getViewModelData` is called with a new `startRow`/`endRow`. Column metadata is already cached - the global resolver is not called again. New pages are fetched with the page resolver's SQL contributions. Evicted pages lose their metadata. The projection step reassembles everything into a fresh `ViewModelMetadata` aligned with the current data block.
...
Example - heatmap renderer using per-column metadata:
...
Example - quality renderer using per-cell metadata:
