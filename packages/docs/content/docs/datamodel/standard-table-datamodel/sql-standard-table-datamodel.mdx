...
`SqlStandardTableDataModel` is the SQL implementation of [StandardTableDataModel](/docs/datamodel/standard-table-datamodel). It takes a [`DataSchema[]`](/docs/api-references/type-references#dataschema), a [`SqlDataSource`](/docs/datasource/sql-datasource), and an optional [`StandardTableConfig`](/docs/api-references/type-references#standardtableconfig), then translates each [`StandardDataFetchAndTransformIR`](/docs/api-references/type-references#standarddatafetchandtransformir) into a single SQL SELECT query.

This implementation targets in-browser WASM SQL engines (like [DuckDB WASM](/docs/datasource/sql-datasource/duckdb-wasm-datasource)) where the grid runs queries locally in the browser. If your data lives behind an API that connects to a full SQL server (Postgres, BigQuery, Redshift, Snowflake etc.), you can use this class as a reference for how to translate the IR into SQL and build your own [`StandardTableDataModel`](/docs/datamodel/standard-table-datamodel#implementing-a-standardtabledatamodel) subclass that generates server-compatible queries.
...
- `config` - optional `Partial<StandardTableConfig>`. Defaults to `{ pageSize: 10000, maxNumPageBeforeEviction: 300 }` if omitted. See [Configuration](/docs/datamodel/standard-table-datamodel#configuration)

The constructor reads the table name from `dataSource.table` and stores it for query generation.

## `getData` implementation

`getData` receives a [`StandardDataFetchAndTransformIR`](/docs/api-references/type-references#standarddatafetchandtransformir) and builds a single SQL query. The query shape depends on whether the IR has grouping active.
...
4. `SUM("revenue")` aggregates measures - the aggregation function comes from the column's [`aggregateFn`](/docs/api-references/type-references#schema) in the schema
...
Each [`ScalarFilter`](/docs/api-references/type-references#scalarfilter) in the IR's `filter` array becomes a WHERE clause. Multiple filters are combined with AND. The supported operators:
...
When a filter has `subtype: "date"`, the model wraps the column with `EXTRACT` before comparing. A [`DatePartScalarFilter`](/docs/api-references/type-references#datepartscalarfilter) has an additional `part` field (`"year"`, `"month"`, `"quarter"`, etc.) that specifies which component to extract:
...
Each [`SortEntry`](/docs/api-references/type-references#sortentry) in the IR's `sort` array becomes an ORDER BY clause. Entries with `direction: "noop"` are skipped. The translation depends on the query mode:
...
Extends [StandardTableDataModel](/docs/datamodel/standard-table-datamodel#api-reference).
