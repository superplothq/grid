...
The layout detects this by comparing the logical scroll position against the viewmodel's `offsetTop` and loaded row count. The event is debounced by [`GridConfig.dataFetchDebounceMs`](/docs/api-references/type-references#gridconfig) (default: 150ms) to prevent flooding the data source during fast scrolling.
