...
| `highlightBaseColor` | `--highlight-base-color` | Accent mixed into a cell's background by the hover effect |
| `hoverColorMixPercentage` | `--hover-color-mix-percentage` | How much of the accent to mix in (0-100) while hovered |
...
grid.matchAll((dim, val) => val === "Total")
  .style((el) => {
