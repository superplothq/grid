...
`SqlDataSource` is an abstract class that builds on top of [DataSource](/grid/docs/datasource). It defines a common abstraction to connect to WASM-based SQL-supported data sources in the browser (like DuckDB WASM or PGLite) and run SQL against them by fixing `execute(sql: string)` to `string`.
...
import { SqlDataSource } from "@superplot/grid";
...
Extends [DataSource](/grid/docs/datasource#api-reference).
