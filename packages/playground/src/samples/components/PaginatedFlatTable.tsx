...
const ExpandCollapseRenderer: React.FC<FacetCellProps> = ({value, path, viewModel, render, flatMeta}) => {
  const {model, expandAction, collapseAction} = useDataModelContext();
  ...
  const handleClick = useCallback(async () => {
    ...
    if (expandAction && collapseAction) {
      if (flatMeta!.isExpanded) await collapseAction(path as string[]);
      else await expandAction(path as string[]);
    } else {
    ...
    setLoading(false);
  }, [model, path, flatMeta, viewModel, render, expandAction, collapseAction]);
...
interface PaginatedFlatTableProps {
  height?: string;
}

const PAGE_SIZE = 25;
const GROUP_BY = ["Agency Name", "Leave Status as of June 30"];

const PaginatedFlatTable: React.FC<PaginatedFlatTableProps> = ({height = "500px"}) => {
  ...
  return <PaginatedFlatTableInner ds={dsState.ds} columns={dsState.columns} theme={theme} height={height} />;
};
...
export default PaginatedFlatTable;
